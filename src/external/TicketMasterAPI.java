...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "OGZghfbyvWzvrywHTLYbqwhppKcoiH3J";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		//String query = String.format("apikey=%s&latlong=%s,%s&keyword=%s&radius=%s", API_KEY, lat, lon, keyword, 5);
		...
		try {
			...
			reader.close();
			JSONObject obj = new JSONObject(response.toString());// response is JSON object format.
	...
	private void queryAPI(double lat, double lon) {
		...
		/*
		 * JSONArray events = search(lat, lon, null); try { for (int i = 0; i <
		 * events.length(); ++i) { JSONObject event = events.getJSONObject(i);
		 * System.out.println(event.toString(2)); } } catch (Exception e) {
		 * e.printStackTrace(); }
		 */
	}
